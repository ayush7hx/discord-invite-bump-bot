...
BUMP_BOTS = {
    ...
    # ── Carl-bot — /bump — 24h cooldown ──────────────────
    "235148962103951360": {
        "name": "Carl-bot",
        "cmd_name": "bump",
        "cmd_id": None,
        "cooldown_h": 24,
        "keywords": ["bumped", "bump", "carl.gg", "server discovery"],
...
async def fetch_guild_bump_commands(guild: discord.Guild):
    ...
    Jo bhi known bump bot server mein milega, uska cmd_id use hoga.
    Agar cmd_id None hai (jaise Carl-bot), to API se dynamically fetch hoga.
    """
    found = {}
    ...
    for app_id, bot_info in BUMP_BOTS.items():
        ...
        if member is not None:
            ...
            if cmd_id is None:
                print(f"[BotDetect] 🔍 {bot_info['name']} found, fetching cmd_id dynamically...")
                cmd_id = await fetch_cmd_id_from_api(guild.id, app_id, bot_info["cmd_name"])
                if cmd_id is None:
                    print(f"[BotDetect] ⚠️ Could not get cmd_id for {bot_info['name']}, skipping.")
                    continue
            found[app_id] = {
                "cmd_id": cmd_id,
                "cmd_version": cmd_id,
            ...
            print(f"[BotDetect] ✅ Found {bot_info['name']} in '{guild.name}' (cmd_id={cmd_id})")
        else:
